                    ...
                    <ul class="nav nav-pills faq__nav" id="pills-tab" role="tablist">
                        @foreach ($categories as $faq_category)
                            <li class="nav-item faq__nav-item my-1" role="presentation">
                                <a onclick="setCategorySlug('{{ $faq_category->slug }}')" href="javascript:void(0)" class="nav-link faq__nav-link @if (request('category')) @if ($faq_category->slug === request('category')) active @endif @else @if ($loop->first) active @endif @endif ">
                                    <span class="icon">
