            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title" style="line-height: 36px;">{{ __('Child Category') }}</h3>
                        @if (true)
                            <a href="{{ route('module.childcategory.create') }}"
                                class="btn bg-primary float-right d-flex align-items-center justify-content-center">
                                <i class="fas fa-plus"></i>&nbsp; {{ __('Add Child Category') }}
                            </a>
                        @endif
                    </div>
                    <div class="card-body">
                                        ...
                                        <thead>
                                            <tr role="row" class="text-center">
                                                ...
                                                <th class="sorting_desc" tabindex="0" aria-controls="example1"
                                                    rowspan="1" colspan="1"
                                                    aria-label="Rendering engine: activate to sort column ascending"
                                                    aria-sort="descending" width="30%">{{ __('category name') }}</th>
                                                <th class="sorting_desc" tabindex="0" aria-controls="example1"
                                                    rowspan="1" colspan="1"
                                                    aria-label="Rendering engine: activate to sort column ascending"
                                            ...
                                            @foreach ($child_categories as $childcategory)
                                                <tr role="row" class="odd">
                                                    <td class="sorting_1 text-center" tabindex="0">
                                                        <a
                                                            href="{{ route('module.childcategory.show', $childcategory->slug) }}">
                                                            {{ $childcategory->name }}
                                                        </a>
                                                    </td>
                                                    <td class="sorting_1 text-center" tabindex="0">
                                                        {{ $childcategory->subcategory->slug ?? '' }}
                                                        {{-- <a href="{{ route('module.childcategory.show', $childcategory->subcategory->slug) }}">{{$childcategory->subcategory->name}}</a> --}}
                                                    </td>
                                                    <td class="text-center">
                                                        <div>
                                                            <label class="switch ">
                                                                <input data-id="{{ $childcategory->id }}" type="checkbox"
                                                        ...
                                                        <td class="sorting_1 text-center" tabindex="0">
                                                            @if (true)
                                                                <a data-toggle="tooltip" data-placement="top"
                                                                    ...
                                                                    href="{{ route('module.childcategory.edit', $childcategory->id) }}"
                                                                    class="btn bg-info"><i class="fas fa-edit"></i></a>
                                                            @endif
                                                            @if (true)
                                                                <form
                                                                    action="{{ route('module.childcategory.destroy', ['childcategory' => $childcategory->id]) }}"
