                            ...
                            <tbody>
                                @forelse ($contacts as $contact)
                                    <tr id="item_id{{ $contact->id }}">
                                        ...
                                        <td>{{ $contact->name }}</td>
                                        <td>{{ $contact->email }}</td>
                                        <td>{{ $contact->subject->title ?? '' }}</td>
                                        <td>{{ $contact->reasone->title ?? '' }}</td>
                                        <td>{{ $contact->created_at }}</td>
                                        <td class="d-flex justify-content-center align-items-center">
                                            {{-- <button contact_id="{{ $contact->id }}" type="submit"
                                                onclick="contactDetail({{ json_encode($contact) }})"
                                                title="{{ __('view_message') }}" class="btn btn-sm bg-info mr-1 msgBtn"><i
...
@section('script')
    ...
    <script>
        function contactDetail(contact) {
            ...
            $('#contact-modal-subject').val(contact.subject.title);
            $('#contact-modal-reason').val(contact.reasone.title);
